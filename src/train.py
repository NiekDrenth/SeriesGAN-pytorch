...
def train(
    ...
    filename_stem = (
        ...
        f"tempdim{temporal_dimension}_"
        f"epochs{num_epoch}_"
    )
    ...
    temporal_optimizer = torch.optim.RMSprop(
        autoencoder.parameters(), lr=1e-4, alpha=0.99
    )
    embedder_optimizer = torch.optim.RMSprop(embedder.parameters(), lr=1e-4, alpha=0.99)
    discriminator_optimizer = torch.optim.RMSprop(
        discriminator.parameters(), lr=1e-4, alpha=0.99
    )
    ae_discriminator_optimizer = torch.optim.RMSprop(
        ae_discriminator.parameters(), lr=1e-4, alpha=0.99
    )
    generator_optimizer = torch.optim.RMSprop(
        generator.parameters(), lr=1e-4, alpha=0.99
    )
    supervisor_optimizer = torch.optim.RMSprop(
        supervisor.parameters(), lr=1e-4, alpha=0.99
    )
    test = False
    ...
    for epoch in range(num_epoch):
        for batch in train_loader:
            ...
            if test:
               ...
               print("Loss value:", temporal_loss.item())
               print("Has NaN in prediction:", torch.isnan(predictions[0]).any())
               print("Has NaN in batch:", torch.isnan(batch).any())
               ...
               #         print(f"{name}: grad max={param.grad.abs().max().item()}")
               # with torch.no_grad():
               #     out = autoencoder(batch)
               #     print("NaN in second forward pass output:", torch.isnan(out[0] if isinstance(out, (tuple, list)) else out).any())
               test = True
    ...
    for epoch in range(num_epoch * 2):
        ...
        for batch in train_loader:
            ...
            G_loss_U_ae_e = mseLoss(
                Y_ae_fake_e_second, torch.ones_like(Y_ae_fake_e_second)
            )
            ...
            embedder_loss = (
                torch.sqrt(
                    ...
                    * 0.1
                    * mseLoss(
                        Y_ae_fake_e_detached, torch.ones_like(Y_ae_fake_e_detached)
                    )
                    + lambda_e
                    * 0.1
                    * mseLoss(
    ...
    torch.save(
        ...
        os.path.join(model_dir, f"autoencoder_{filename_stem}.pt"),
    )
    torch.save(
        embedder.state_dict(), os.path.join(model_dir, f"embedder_{filename_stem}.pt")
    )
    torch.save(
        discriminator.state_dict(),
        os.path.join(model_dir, f"discriminator_{filename_stem}.pt"),
    )
    torch.save(
        ae_discriminator.state_dict(),
        os.path.join(model_dir, f"ae_discriminator_{filename_stem}.pt"),
    )
    torch.save(
        generator.state_dict(), os.path.join(model_dir, f"generator_{filename_stem}.pt")
    )
    torch.save(
        supervisor.state_dict(),
        os.path.join(model_dir, f"supervisor_{filename_stem}.pt"),
    )
...
if __name__ == "__main__":
    ...
    def parse_args():
        parser = argparse.ArgumentParser(description="Parse arguments for your model.")

        parser.add_argument(
            "--seq_len", type=int, default=24, help="Sequence length (default: 24)"
        )
        parser.add_argument(
            "--batch_size", type=int, default=100, help="Batch size (default: 100)"
        )
        parser.add_argument(
            "--dim", type=int, default=6, help="Dimension size (default: 6)"
        )
        parser.add_argument(
            "--num_layers", type=int, default=4, help="Number of layers (default: 4)"
        )
        parser.add_argument(
            "--temporal_dimension",
            type=int,
        ...
        parser.add_argument(
            "--num_epoch", type=int, default=50, help="Number of epochs (default: 50)"
        )
        parser.add_argument(
            "--filename",
            type=str,
